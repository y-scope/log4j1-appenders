...
/**
 * This abstract class enforces an opinionated workflow, skeleton interfaces
 * and hooks optimized towards buffered rolling file appender implementations
 * with remote persistent storage. For example, an appender which emits logs
 * events to S3 object store while also utilizing streaming compression to
 * compress the log events as they are generated. To keep the logs on remote
 * persistent storage relatively in sync with logs produced by the appender
 * without the need to frequently flush the buffer, an implementation of
 * verbosity-aware hard+soft timeout based log freshness policy is provided.
 * With this mechanism, derived classes can enjoy the benefit of keeping remote
 * log file synchronized efficiently.
 */
        ...
        implements Flushable
{
    ...
    /**
     * Method invoked by log4j library via reflection or manually by the user
     * to set the hard flush timeout of various log verbosities via a csv
     * string with the key being VERBOSITY string and value being minutes:
    ...
    /**
     * Method invoked by log4j library via reflection or manually by the user
     * to set the soft flush timeout of variouss log verbosities via a csv
     * string with the key being VERBOSITY string and value being seconds:
    ...
    /**
     * Method invoked by log4j library via reflection or manually by the user
     * to disable (default enable) the closing of files upon receiving a
     * shutdown signal prior to JVM exit. This is a rarely used bu useful
     * functionality when user wants to capture as much log events as possible
     * after a shutdown signal is received at the risk of data loss.
    ...
    /**
     * Method invoked by log4j library via reflection or manually by the user
     * to adjust the amount of time which the background sync thread sleeps
    ...
    /**
     * In some cases, such as unit testing, we want to explicitly set the
     * deadline parameters to precisely control flushing behavior
     * @param epochMs
     */
    ...
    /**
     * Allows log4j or derived classes to activate the options in this class.
     * In addition, this method also starts two background threads used to
     * asynchronously flush output buffer to file and synchronize them to
     * remote persistent storage. A shutdown hook is installed to gracefully
     * shut down the appender if {@code closeFileOnShutdown} is {@code true}.
     * Otherwise, the appender will keep on appending logs until JVM exit
     * and make the best effort to upload logs. User will accept the risk
     * higher chance of data-loss, but gains the chance to capture logs
     * after shutdown hook is invoked.
     */
    @Override
    ...
    /**
     * The implementation shall append the log event into derived class's
    ...
    /**
     * Reset freshness parameter means increasing the deadline to infinity.
     * Only when a new log message is appended will the deadline be updated.
     */
     ...
     * that this method is marked as synchronized because file flushing could
     * be called in the background in the {@code BackgroundFlushThread}. We
     * must ensure flush does not have data race with append operations.
     * Synchronization after the flush is also processed in the background as
     * to ensure actual log append operation is unblocked as soon as possible.
     * @throws IOException
     */
     ...
     * rollover operation. This method is intentionally made to be final to
    ...
    /**
     * The implementation of this abstract sync method shall fulfill the duty
     * of managing on-disk and/or remote log file life-cycles.
     * For example: upload files to remote storage while retaining 3 most
     * recently used log files on local on-disk storage.
     ...
     * thread responsible for flushing and synchronization. Note that similar
     * to {@code append()}, this method is also intentionally made to be final
     * to ensure it is not over-ridden by derived class. To allow for
     * user-defined behaviors, user should override the following hook method:
    ...
    private class BackgroundFlushThread extends Thread {
        ...
        public void run() {
            while (true) try {
                ...
                if (closeFileOnShutdown) {
                    logDebug("Received interrupt message for " +
                            "graceful shutdown of BackgroundFlushThread");
                    break;
                }
            }
        }
     ...
     * synchronization. To customize the execution thread's behavior, over-ride
    ...
    private class BackgroundSyncThread extends Thread {
        ...
        public void submitShutdownRequest() {
            ...
            while (true) if (backgroundSyncRequests.offer(shutdownRequest)) {
                break;
            }
        }
        ...
        public void submitSyncRequest(String path, boolean deleteFile) {
            ...
            while (true) if (backgroundSyncRequests.offer(syncRequest)) {
                break;
            }
        }
